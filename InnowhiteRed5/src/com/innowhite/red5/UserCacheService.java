...
public class UserCacheService {
	...
	private static Logger log = Red5LoggerFactory.getLogger(UserCacheService.class, InnowhiteConstants.APP_NAME);

	// has mapping of roomId vs list of users in the audio conference.
	static ConcurrentHashMap<String, List<String>> userAudioConfMap = new ConcurrentHashMap<String, List<String>>();
	...
	public static void addConfRoom(String confNumber, String roomID) {
		...
		log.debug(" adding  confNumber " + confNumber + " roomID " + roomID + "   to map " + confRoom);
	}
	...
	public static void addInnoUniqueIDUser(String innoUniqueId, String actualUserID) {
		...
		log.debug(" adding  innoUniqueId " + innoUniqueId + " actualUserID " + actualUserID + "   to map " + innoUniqueIDUser + "  " + innoUniqueIDUser.get(innoUniqueId));
	 ...
	 * Get the actual userId from innoUniqueIDUser and set in the con
	 */
	public static void addparticipantIDUser(String participantId, String innoUniqueId) {

		innoUniqueId = innoUniqueId.substring(0, 6);

		log.debug(" adding  participantId " + participantId + " innoUniqueId " + innoUniqueId + "   to map " + innoUniqueIDUser + "  the length of map is :" + innoUniqueIDUser.size());
		String userId = innoUniqueIDUser.get(innoUniqueId);
		if (userId != null)
			...
			while (itr.hasNext()) {
				...
				log.warn("key :: " + key + " value :: " + innoUniqueIDUser.get(key) + "  my key  " + innoUniqueId + "  length  " + innoUniqueId.length() + "  and key length " + key.length());
			}
	...
	public static String getUseId(String participantId) {
		...
		if (result == null) {
			...
			log.warn("getUseId -- the result of for lookup participantId ::" + participantId + "  in map ::" + participantIDUser);
		}
	...
	public static String getActualRoom(String confId) {
		...
		if (result == null) {
			...
			log.warn("getActualRoom -- the result of for lookup confId ::" + confId + "  in map ::" + confRoom);
		}

		return result;
	}

	public static void addRoomIdUserSharedObj(String name, ISharedObject sharedObject) {

		log.debug(" adding to the map name " + name + "  shared obj " + sharedObject);
	...
	public static ISharedObject getUserSharedObj(String roomId) {
		...
		if (result == null) {
			log.warn("getUserSharedObj is null  roomId " + roomId + "   map is::" + roomIdUserSharedObj);
		}
	...
	public static boolean roomExists(String roomId) {
		...
		if (roomId != null) {
			if (confRoom.containsValue(roomId) == true)
				...
				log.warn(" the roomId " + roomId + " does not exist in " + confRoom);
				return false;
			}
		}
		return true;
	...
	/*
	 * This function returns true of the user is in the audio conference.
	 */
	public static boolean userExistsInConf(String roomId, String userId) {

		log.debug("enterd userExistsInConf ");
		log.debug("roomID " + roomId + " userId " + userId + " list:  " + userAudioConfMap.get(roomId));

		List<String> users = userAudioConfMap.get(roomId);
	...
	/*
     * */
	public static void addUserConfMap(String roomId, String userId) {
		if (userAudioConfMap.get(roomId) == null) {
			List<String> users = new ArrayList<String>();
			userAudioConfMap.put(roomId, users);
		}
	...
	/*
     * */
	public static void removeUserConfMap(String roomId, String userId) {
		List<String> users = userAudioConfMap.get(roomId);
		users.remove(userId);
	}

	public static void removeRoomConfMap(String roomId) {
		userAudioConfMap.remove(roomId);
	}
