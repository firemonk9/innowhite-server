...
public class PlaybackDataService {
	...
	private static final Logger log = LoggerFactory
			.getLogger(PlaybackDataService.class);
	...
	public void process(String roomId, boolean upload) {
		...
		try {
			...
			List<SessionRecordings> sessionRecordingsList = sessionRecordingsDao
					.getSessionRecordingList(roomId);

			if (sessionRecordingsList != null
					&& sessionRecordingsList.size() == 0) {
			...
			List<AudioData> audioDataList = audioDataDao
					.getAudioDataList(roomId);

			// VideoDataDao videoDataDao = (VideoDataDao)
			// factory.getBean("videoDataDao");
			// TODO remove videos with (<=)3seconds duration
			List<VideoData> videoDataList = videoDataDao
					.getVideoDataList(roomId);
			for (int i = 0; i < videoDataList.size(); i++) {

				if (videoDataList.get(i) == null
						|| videoDataList.get(i).getStartTime() == null
						|| videoDataList.get(i).getEndTime() == null) {
					log.warn(" video cannot have bacis information null so exiting the process.  ");
					log.warn(" Getting invalid information for video obj "
							+ videoDataList.get(i));
				...
				if ((vE - vS) < 3) {
					...
					log.debug("removing video at index:" + i
							+ " with end-time:" + vE + " and start-time:" + vS);
				}
			}
			...
			if (PlaybackUtil.isWindows()) {
				...
				PlaybackUtil.updateVideoPathWindows(
						playbackVO.getWinVideoPath(), videoDataList);
				PlaybackUtil.updateAudioPathWindows(
						playbackVO.getWinAudioPath(), audioDataList);
			}
			if (PlaybackUtil.isUbuntu()) {
				...
				// videoDataList);
				PlaybackUtil.updateAudioPathWindows(
						playbackVO.getUbuntuAudioPath(), audioDataList);
			}
			...
			log.debug("--------------------------------------------------------------");
			log.debug("Number of sessions in room " + roomId + " :: "
					+ sessionRecordingsList.size());
			for (int i = 0; i < sessionRecordingsList.size(); i++) {
				log.debug("Session " + i + " start-time:: "
						+ sessionRecordingsList.get(i).getStartTime());
				log.debug("Session " + i + " end-time:: "
						+ sessionRecordingsList.get(i).getEndTime());
			}
			log.debug("Number of audios in room " + roomId + " :: "
					+ audioDataList.size());
			for (int i = 0; i < audioDataList.size(); i++) {
				log.debug("Audio " + i + " start-time:: "
						+ audioDataList.get(i).getStartTime());
				log.debug("Audio " + i + " end-time:: "
						+ audioDataList.get(i).getEndTime());
				log.debug("Audio " + i + " file-path:: "
						+ audioDataList.get(i).getFilePath());
			}
			log.debug("Number of videos in room " + roomId + " :: "
					+ videoDataList.size());
			for (int i = 0; i < videoDataList.size(); i++) {
				log.debug("Video " + i + " start-time:: "
						+ videoDataList.get(i).getStartTime());
				log.debug("Video " + i + " end-time:: "
						+ videoDataList.get(i).getEndTime());
				log.debug("Video " + i + " file-path:: "
						+ videoDataList.get(i).getFilePath());
			}
			log.debug("--------------------------------------------------------------");
			...
			String[] dimArr = getMaxVideoDimensions(videoDataList, videohm)
					.split("##");
			...
			if (sessionRecordingsList.size() == 0) {
				...
				log.debug("--------------------------------------------------------------");
				for (int i = 0; i < sessionRecordingsList.size(); i++) {
					long sessionStartTime = sessionRecordingsList.get(i)
							.getStartTime().getTime();
					long sessionEndTime = sessionRecordingsList.get(i)
							.getEndTime().getTime();
					log.debug("session Start Time::"
							+ sessionRecordingsList.get(i).getStartTime());
					log.debug("session End Time::"
							+ sessionRecordingsList.get(i).getEndTime());
					...
					if (audioDataList.size() == 0) {
						...
						for (int j = 0; j < audioDataList.size(); j++) {
							...
							prepareAudioForSessionBucket(sb, j,
									audioDataList.get(j), sessionStartTime,
									sessionEndTime);
						}
					...
					if (videoDataList.size() == 0) {
						...
						for (int j = 0; j < videoDataList.size(); j++) {
							...
							prepareVideoForSessionBucket(sb, j,
									videoDataList.get(j), sessionStartTime,
									sessionEndTime);
						}
				...
				while (sessionKeys.hasNext()) {
					...
					sessionCounter++;
					SessionRecordings session = (SessionRecordings) sessionKeys
							.next();
					long sessionStartTime = session.getStartTime().getTime();
					Date sst = session.getStartTime();
					log.debug("sessionStartTime(long)::::: "
							+ PlaybackUtil.secondsToHours(sessionStartTime)
							+ " sessionStartTime(Date)::::" + sst);
					...
					List<AudioData> sessionAudioDataList = sessionBucket
							.getAudioDataList();
					...
					if (sessionAudioDataList.size() > 0) {
						paddedSessionAudioDataList = padAudioPlaylist(
								sessionAudioDataList, sessionStartTime);
						log.debug("_______________________________________________________________");
						log.debug("Number of audios after padding :: "
								+ paddedSessionAudioDataList.size());
						for (int i = 0; i < paddedSessionAudioDataList.size(); i++) {
							log.debug("Audio "
									+ i
									+ " start-time:: "
									+ paddedSessionAudioDataList.get(i)
											.getStartTime());
							log.debug("Audio "
									+ i
									+ " end-time:: "
						...
						sessionAudio = concatenateAudios(
								paddedSessionAudioDataList, sessionCounter);
					} else {
						if (audioDataList.size() > 0) {
							log.debug("Session has no audios. Since, room has audios.. creating silent audio for this session.");
							sessionAudio = createSilentAudio(sessionEndTime
									- sessionStartTime);
						} else {
					...
					if (sessionAudio != null) {
						log.debug("Final session audio path::"
								+ sessionAudio.getFilePath());
					} else {
					...
					List<VideoData> sessionVideoDataList = sessionBucket
							.getVideoDataList();
					...
					if (sessionVideoDataList.size() > 0) {
						...
						if (screenShareFlag.equals("true")) {
							...
							paddedSessionVideoDatalist = padSessionVideoPlaylist(
									sessionVideoDataList, maxVideoDimensions);
						...
						log.debug("_______________________________________________________________");
						log.debug("Number of videos after setting resolution :: "
								+ uniformSessionVideoDataList.size());
						for (int i = 0; i < uniformSessionVideoDataList.size(); i++) {
							log.debug("Video "
									+ i
									+ " start-time:: "
									+ uniformSessionVideoDataList.get(i)
											.getStartTime());
							log.debug("Video "
									+ i
									+ " end-time:: "
						...
						sessionVideo = concatenateVideos(
								uniformSessionVideoDataList, sessionCounter);
					}
					...
					log.debug("FINAL SESSION VIDEO PATH::"
							+ sessionVideo.getFilePath());
				...
				String flowPlayerVideoPath = flowPlayerVideo(listPlayback,
						roomId);
				PlayBackPlayList flowPlayerVideo = setPlayBackPlayList(
						flowPlayerVideoPath, roomId);
				...
				if (upload) {
					...
					if (youtubeURL != null && youtubeURL.length() > 0) {
						CallBackUrlsData callBackVO = callBackUrlsDao
								.getURLData(roomId);
						String url = null;
						if (callBackVO != null) {
	...
	private List<VideoData> setDimensionsSessionVideoPlaylist(
			List<VideoData> paddedSessionVideoDatalist,
			String maxVideoDimensions, PlaybackVO playbackVO2) {
		...
		for (int i = 0; i < paddedSessionVideoDatalist.size(); i++) {
			...
			sameDimensionSessionVideoDatalist.add(paddedSessionVideoDatalist
					.get(i));
	...
	private String flowPlayerVideo(List<PlayBackPlayList> listPlayback,
			String roomId) {
		...
		String flowPlayerVideoPath = listPlayback.get(0).getFilePath()
				.replace(".flv", "flowPlayerVideo" + roomId + ".flv");
		;
		String cmd = null;
		if (listPlayback.size() > 1) {
			...
			log.debug("listPlayback.size() <= 1");
			cmd = " -i " + listPlayback.get(0).getFilePath()
					+ " -acodec copy -vcodec copy -ar 44100 -ab 64k "
					+ flowPlayerVideoPath;
			log.debug("Ffmpeg Command for concatenating just the 1 video::: (renaming file)"
					+ cmd);
			PlaybackUtil.invokeFfmpegProcess(cmd);
		}
	...
	private List<VideoData> padSessionVideoPlaylist(
			List<VideoData> sessionVideoDataList, String maxVideoDimensions) {
		...
		for (int i = 0; i < sessionVideoDataList.size(); i++) {
			...
			if (videoType != null && videoType.equals("DESKTOP")) {
				...
				long start_time = sessionVideoDataList.get(i).getStartTime()
						.getTime();
				long end_time = sessionVideoDataList.get(i).getEndTime()
						.getTime();
				long dbDuration = (end_time - start_time) / 1000;
				long actualDuration = PlaybackUtil.getNumLong(vhm
						.get("duration"));
				...
				log.debug(" ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
				log.debug(" start time for screen share video:: "
						+ PlaybackUtil.secondsToHours(start_time));
				log.debug(" end time for screen share video:: "
						+ PlaybackUtil.secondsToHours(end_time));
				log.debug(" ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
				log.debug(" Actual (ffmpeg) video duration :: "
						+ actualDuration);
				log.debug(" Expected (database) video duration :: "
						+ dbDuration);
				...
				cmd = " -i " + SSPaddingVideoPath + " -t "
						+ PlaybackUtil.secondsToHours(padDuration * 1000)
						+ " -ar 44100 -ab 64k " + curDir + "/SSPad"
						+ uniquePath + ".flv";
				...
				vd.setFilePath(curDir + "/SSPad" + uniquePath + width + "x"
						+ height + ".flv");
	...
	private String getMaxVideoDimensions(List<VideoData> videoList,
			HashMap<String, String> vhm) {
	...
	private String getScreenShareDimensions(List<VideoData> videoList,
		...
		return videohm.get("width") + "x" + videohm.get("height") + "##"
				+ screenShareFlag;
	}

	private String mergeAudioVideo(AudioData sessionAudio,
			VideoData sessionVideo) {
		...
		cmd = " -i " + sessionAudio.getFilePath() + " -ar 44100 -ab 64k "
				+ sessionAudio.getFilePath().replace(".mp3", ".avi");
		PlaybackUtil.invokeFfmpegProcess(cmd);
		...
		cmd = " -i "
				...
				+ " -i "
				+ sessionAudio.getFilePath().replace(".mp3", ".avi")
				+ " -ar 44100 -ab 64k -vcodec copy "
				+ sessionVideo.getFilePath().replace(".avi",
						newVideoPath + "playlist.avi");
		log.debug(":::merging final session audio and final session video:::"
				+ cmd);
		PlaybackUtil.invokeFfmpegProcess(cmd);
		// long duration = sessionVideo.getEndTime().getTime() -
		// sessionVideo.getStartTime().getTime();
		return sessionVideo.getFilePath().replace(".avi",
				newVideoPath + "playlist.avi");// "##" + duration;
	}

	private AudioData concatenateAudios(
			List<AudioData> paddedSessionAudioDataList, int sessionCounter) {
		log.debug("Inside concatenateAudios..............");
		AudioData ad = new AudioData();
		String sessionAudioPath = paddedSessionAudioDataList.get(0)
				.getFilePath()
				.replace(".mp3", "SessionAudio" + sessionCounter + ".mp3");
		if (paddedSessionAudioDataList.size() > 1) {
			...
			for (int i = 0; i < paddedSessionAudioDataList.size(); i++) {
				cmd = cmd + " "
						+ paddedSessionAudioDataList.get(i).getFilePath();
			}
			log.debug("Mp3Wrap Command for concatenating audios:::" + cmd);
			PlaybackUtil.invokeMp3Process(cmd);
			ad.setEndTime(paddedSessionAudioDataList.get(
					paddedSessionAudioDataList.size() - 1).getEndTime());
			ad.setFilePath(sessionAudioPath.replace(".mp3", "_MP3WRAP.mp3"));
		} else {
			String cmd = " -i "
					+ paddedSessionAudioDataList.get(0).getFilePath() + " "
					+ sessionAudioPath;
			log.debug("Ffmpeg Command for concat 1 audio::: (renaming file)"
					+ cmd);
			PlaybackUtil.invokeFfmpegProcess(cmd);
	...
	private ArrayList<VideoData> setVideoFormatResolution(
			List<VideoData> sessionVideoDataList, String screenShareDimensions) {
		...
		for (int i = 0; i < sessionVideoDataList.size(); i++) {
			vd = new VideoData();
			long a = (sessionVideoDataList.get(i).getEndTime().getTime() - sessionVideoDataList
					.get(i).getStartTime().getTime());
			log.debug("::::DURATION::::" + PlaybackUtil.secondsToHours(a));
			// screenShareDimension
			cmd = " "
					+ sessionVideoDataList.get(i).getFilePath()
					+ " -oac copy -ovc lavc -vf scale="
					+ dim[0]
					+ ":"
					+ dim[1]
					+ " -o "
					+ sessionVideoDataList.get(i).getFilePath()
							.replace(".flv", dim[0] + "x" + dim[1] + ".avi");
			log.debug("::::MenCoder Command for setting resolution of video"
					+ i + "/" + sessionVideoDataList.size() + "::::" + cmd);
			PlaybackUtil.invokeMencoderProcess(cmd);
	...
	private VideoData concatenateVideos(
			List<VideoData> uniformSessionVideoDataList, int sessionCounter) {
		log.debug("Inside concatenateVideos..............");
		// mencoder.exe -oac copy -ovc lavc wb_audio3.avi
		// screen_share_audio3.avi -o complete33.avi
		VideoData vd = new VideoData();
		String sessionVideoPath = uniformSessionVideoDataList.get(0)
				.getFilePath()
				.replace(".flv", "SessionVideo" + sessionCounter + ".avi");
		;
		if (uniformSessionVideoDataList.size() > 1) {
			String cmd = " -oac copy -ovc copy ";
			for (int i = 0; i < uniformSessionVideoDataList.size(); i++) {
				cmd = cmd + " "
						+ uniformSessionVideoDataList.get(i).getFilePath();
			}
			cmd = cmd + " -o " + sessionVideoPath;
			log.debug("MenCoder Command for concatenating videos:::" + cmd);
			PlaybackUtil.invokeMencoderProcess(cmd);
			vd.setEndTime(uniformSessionVideoDataList.get(
					uniformSessionVideoDataList.size() - 1).getEndTime());
		} else {
			String cmd = " -i "
					+ uniformSessionVideoDataList.get(0).getFilePath() + " "
					+ sessionVideoPath;
			log.debug("Ffmpeg Command for concatenating just the 1 video::: (renaming file)"
					+ cmd);
			PlaybackUtil.invokeFfmpegProcess(cmd);
	...
	private List<AudioData> padAudioPlaylist(
			List<AudioData> sessionAudioDataList, long sessionStartTime) {
		...
		for (int i = 0; i < sessionAudioDataList.size(); i++) {
			ad = new AudioData();
			long audioStartTime = sessionAudioDataList.get(i).getStartTime()
					.getTime();
			log.debug("audioStartTime: "
					+ sessionAudioDataList.get(i).getStartTime());
			log.debug("sessionStartTime: "
					+ PlaybackUtil.secondsToHours(sessionStartTime));
			...
			if (i == 0) {
				...
				long prevAudioEndTime = sessionAudioDataList.get(i - 1)
						.getEndTime().getTime();
				if ((audioStartTime - prevAudioEndTime) > 0) {
					...
					String silentAudioPath = createSilentAudio(
							audioStartTime - prevAudioEndTime).getFilePath();
					log.debug("silentAudioPath: " + silentAudioPath);
		...
		log.debug("tempSessionAudioPlaylist.size()"
				+ tempSessionAudioPlaylist.size());
		return tempSessionAudioPlaylist;
	}

	private AudioData createSilentAudio(long duration) {
		log.debug("Inside createSilentAudio of " + duration
				+ " seconds..............");
		AudioData ad = new AudioData();
		...
		String cmd = null;
		if (duration <= 7200000) {
			cmd = " -i " + silentAudioPath + " -ss 00:00:00 -t "
					+ PlaybackUtil.secondsToHours(duration)
					+ " -ar 44100 -ab 64k "
					+ silentAudioPath.replace(".mp4", newAudioPath + ".mp3");
		} else {
			log.debug("cannot create silent audio of " + duration
					+ " (>7200000)seconds");
		}
	...
	private String convertAVItoMP4264(String avi_filepath) {
		String cmd = " -i "
				+ avi_filepath
				+ " -vcodec libx264 -g 250 -bf 3 -b_strategy 1 -coder 1 -qmin 10 "
	...
	private String convertAVItoFLV(String avi_filepath) {
		...
		String cmd = " -i " + avi_filepath
				+ " -acodec copy -vcodec copy -ar 44100 -ab 64k "
				+ avi_filepath.replace(".avi", ".flv");
		PlaybackUtil.invokeFfmpegProcess(cmd);
		String flv_filepath = avi_filepath.replace(".avi", ".flv");
		return flv_filepath;
	}

	private static void prepareVideoForSessionBucket(SessionBucket sb, int j,
			VideoData videoData, long sessionStartTime, long sessionEndTime) {
		log.debug("-----------------------------------------");
		...
		if (videoStartTime <= sessionStartTime
				&& videoEndTime <= sessionEndTime
				&& videoEndTime >= sessionStartTime) {
			...
			executeFfmpegAndSetVideoData(cmd, vd, sessionStartTime,
					videoEndTime,
					videoPath.replace(".flv", newVideoPath + ".flv"));
		} else if (videoStartTime <= sessionStartTime
				&& videoEndTime >= sessionEndTime) {
			...
			cmd = "-i "
					...
					+ videoPath.replace(".flv", newVideoPath + ".flv");
			executeFfmpegAndSetVideoData(cmd, vd, videoStartTime, videoEndTime,
					videoPath.replace(".flv", newVideoPath + ".flv"));
		} else if (videoStartTime >= sessionStartTime
				&& videoStartTime <= sessionEndTime
				&& videoEndTime >= sessionEndTime) {
			...
			cmd = "-i "
					...
					+ videoPath.replace(".flv", newVideoPath + ".flv");
			executeFfmpegAndSetVideoData(cmd, vd, videoStartTime,
					sessionEndTime,
					videoPath.replace(".flv", newVideoPath + ".flv"));
		} else {
		...
		if (sessionBucketFlag) {
			...
			if (vd != null) {
				...
				PlaybackUtil.printVals(videoStartTime, sessionStartTime,
						videoEndTime, sessionEndTime);
			}
	...
	private static void prepareAudioForSessionBucket(SessionBucket sb, int j,
			AudioData audioData, long sessionStartTime, long sessionEndTime) {
		log.debug("-----------------------------------------");
		...
		if (audioStartTime <= sessionStartTime
				&& audioEndTime <= sessionEndTime
				&& audioEndTime >= sessionStartTime) {
			...
			cmd = " -i "
					...
					+ PlaybackUtil.secondsToHours(sessionStartTime
							- audioStartTime)
					+ " -t "
					+ PlaybackUtil.secondsToHours(audioEndTime
							- sessionStartTime) + " -ar 44100 -ab 64k "
					+ audioPath.replace(".wav", newAudioPath + ".mp3");
			executeFfmpegAndSetAudioData(cmd, ad, sessionStartTime,
					audioEndTime,
					audioPath.replace(".wav", newAudioPath + ".mp3"));
		} else if (audioStartTime <= sessionStartTime
				&& audioEndTime >= sessionEndTime) {
		...
		// audio in b/w the session
			...
			cmd = " -i "
					...
					+ " -ss 00:00:00 -t "
					+ PlaybackUtil
							.secondsToHours(audioEndTime - audioStartTime)
					+ " -ar 44100 -ab 64k "
					+ audioPath.replace(".wav", newAudioPath + ".mp3");
			executeFfmpegAndSetAudioData(cmd, ad, audioStartTime, audioEndTime,
					audioPath.replace(".wav", newAudioPath + ".mp3"));
		}
		// starts in b/w and ends after
			...
			cmd = " -i "
					...
					+ " -ss 00:00:00 -t "
					+ PlaybackUtil.secondsToHours(sessionEndTime
							- audioStartTime) + " -ar 44100 -ab 64k "
					+ audioPath.replace(".wav", newAudioPath + ".mp3");
			executeFfmpegAndSetAudioData(cmd, ad, audioStartTime,
					sessionEndTime,
					audioPath.replace(".wav", newAudioPath + ".mp3"));
		} else {
			sessionBucketFlag = false;
	...
	private static void executeFfmpegAndSetAudioData(String cmd, AudioData ad,
			long start_time, long end_time, String audio_path) {
		PlaybackUtil.invokeFfmpegProcess(cmd);
	...
	private static void executeFfmpegAndSetVideoData(String cmd, VideoData vd,
			long start_time, long end_time, String video_path) {
		PlaybackUtil.invokeFfmpegProcess(cmd);
		vd.setStartTime(new Date(start_time));
	...
	@SuppressWarnings("unused")
	private static ArrayList<String> mapAudioToVideoStartBetween(
			List<AudioData> audioList, int j, List<VideoData> videos,
			long sessionStartTime, long sessionEndTime) {
		log.debug("--------------------------------------------");
		...
		if (videoStartIndex == videoStopIndex) {
			...
			if (audioStartTime > videoStartTime) {
				...
				if (j == 0) {
					...
					cmd = "-i "
							+ videos.get(videoStartIndex).getFilePath()
							+ " -ss 00:00:00 -t "
							+ PlaybackUtil.secondsToHours(audioStartTime
									- videoStartTime)
							+ " -ar 44100 -ab 64k "
							+ videos.get(videoStartIndex)
									.getFilePath()
									.replace(".avi",
					...
					if (nextAudioStartTime != 0) {
						...
						cmd = "-i "
								...
								+ " -t "
								+ PlaybackUtil
										.secondsToHours(nextAudioStartTime
												- audioStartTime)
								+ " "
								+ videos.get(videoStartIndex)
										.getFilePath()
					...
					// cut/merge the video till current video ends
						...
						log.debug("if(nextAudioStartTime==0)");
						newVideoPath = PlaybackUtil.getUnique();
						duration = videos.get(videoStartIndex).getEndTime()
								.getTime()
								- videos.get(videoStartIndex).getStartTime()
										.getTime();
						log.debug(":::::DURATION:::::" + duration);
						cmd = "-i "
								+ videos.get(videoStartIndex).getFilePath()
					...
					cmd = "-i "
							...
							+ videos.get(videoStartIndex)
									.getFilePath()
									.replace(".avi",
											"playlist" + newVideoPath1 + ".avi");
					PlaybackUtil.invokeFfmpegProcess(cmd);
					finalVideoPlaylist.add(videos
							.get(videoStartIndex)
							.getFilePath()
							.replace(
									".avi",
				...
				if (nextAudioStartTime != 0) {
					...
					cmd = "-i "
							...
							+ PlaybackUtil.secondsToHours(audioStartTime
									- videoStartTime)
							+ " -t "
							+ PlaybackUtil.secondsToHours(nextAudioStartTime
									- audioStartTime)
							+ " "
							+ videos.get(videoStartIndex)
									.getFilePath()
									.replace(".avi",
											"int_play" + intVideoPath + ".avi");
					PlaybackUtil.invokeFfmpegProcess(cmd);
					log.debug(cmd);
				...
				if (audioStartTime >= videoStartTime
					...
					if (audioStartTime == videoStartTime) {
						...
						cmd = "-i "
								...
								+ audioPath
								+ " -ar 44100 -ab 64k "
								+ videoPath.replace(".avi", "playlist"
										+ newVideoPath + ".avi");
						PlaybackUtil.invokeFfmpegProcess(cmd);
						log.debug(cmd);
						finalVideoPlaylist.add(videoPath
								.replace(".avi", "playlist" + newVideoPath
										+ ".avi##" + duration));
					} else if (audioStartTime > videoStartTime) {
						log.debug("audioStartTime > videoStartTime");
						...
						cmd = "-i "
								...
								+ PlaybackUtil.secondsToHours(audioStartTime
										- videoStartTime)
								+ " "
								+ videoPath.replace(".avi", "init_play"
										+ newVideoPath + ".avi");
						PlaybackUtil.invokeFfmpegProcess(cmd);
						log.debug(cmd);
						...
						PlaybackUtil.invokeFfmpegProcess(cmd);
						log.debug(cmd);
						finalVideoPlaylist.add(videoPath.replace(".avi",
								"playlist" + newVideoPath1 + ".avi##"
										+ duration));
						//
						// String newAudioPath = PlaybackUtil.getUnique();
					...
					cmd = "-i "
							...
							+ PlaybackUtil.secondsToHours(videoEndTime
									- audioStartTime)
							+ " "
							+ audioPath.replace(".avi", "init_play"
									+ newAudioPath + ".avi");
					PlaybackUtil.invokeFfmpegProcess(cmd);
					log.debug(cmd);
					...
					PlaybackUtil.invokeFfmpegProcess(cmd);
					log.debug(cmd);
					finalVideoPlaylist.add(videoPath.replace(".avi", "playlist"
							+ newVideoPath + ".avi##" + duration));
				} else if (audioStartTime < videoStartTime
						&& audioEndTime > videoEndTime) {
					...
					cmd = "-i "
							+ audioPath
							+ " -ss "
							+ PlaybackUtil.secondsToHours(videoStartTime
									- audioStartTime)
							+ " -t "
							+ PlaybackUtil.secondsToHours(videoEndTime
									- videoStartTime) + " "
	...
	private static void updateFinalVideoTable(PlayBackPlayList flowPlayerVideo,
			PlayBackPlayListDao playBackPlayListDao) {
