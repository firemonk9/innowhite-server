...
public class ProcessConversion {
	...
	private static final Logger log = LoggerFactory.getLogger(ProcessConversion.class);
	...
	public static String lessonDocumentConverter(String documentId, String serverURL, String orgName) {
		...
		String documentsource = InnowhiteProperties.getPropertyVal("MEDIA_PATH") + filePath.substring(filePath.lastIndexOf("/") + 1);

		log.debug(" documentId " + documentId + "  documentsource " + documentsource);

		File documentFile = new File(documentsource);

		String fileName = documentsource.substring(documentsource.lastIndexOf("/") + 1);
		...
		if ((Utility.allowedFiles(fileName) == true)) {
			...
			if (documentFile.exists()) {
				...
				return convertDocument(documentFile.getAbsolutePath(), documentId, "", fileName, serverURL, false, fileName, true);
	...
	public static String convertDocument(String filePath, String user, String desc, String documentName, String serverURL, boolean saveInDB, String origFileName, boolean isInet) {
		...
		log.debug(" filePath: " + filePath + " user: " + user + "  desc: " + desc + "  documentName: " + documentName);
		...
		if (fileIsPPT(documentName) == true) {

			String conversionId = processPPTX(filePath, user, desc, documentName, serverURL, saveInDB, origFileName);
			return conversionId;
		}
	...
	// on server.
	private static String processPPTX(String filePath, String user, String desc, String documentName, String serverURL, boolean saveInDB, String origFileName) {
		...
		try {
			...
			// location.
			String shraedPath = InnowhiteProperties.getPropertyVal(InnowhiteConstants.SHARED_PATH);
			...
			if (dirCreated == true) {
				...
				MakeExectuable.setWriteAll(dir.getAbsolutePath());	
				if (filePath.endsWith(".pdf")) {
					count = getNumPages(filePath);
				}
				...
				log.debug(" num pages in pdf is ::   " + count);
	...
	private static int getNumPages(String pdfFileAbsPath) {
		//
		...
		String[] Command = null;
		// String urlPath = Constants.TEMP_LOCATION + ext + ".sh";
		// urlPath = urlPath.replaceAll(Constants.APP_NAME,
	...
	private static boolean fileIsPPT(String documentName) {
		...
		return Utility.allowedFilesProceessedonServer(documentName);
	...
	private static void createImagesfromPDF(String inputFile, String user, String desc, String documentName) {
		...
		try {
			...
			urlPathUbuntu = urlPathUbuntu.replaceAll(Constants.APP_NAME, InnowhiteConstants.CONTEXT_PATH);

			String outPutDir = urlPath;

			boolean val = (new File(outPutDir)).mkdir();

			sr.append("convert" + " " + urlPathUbuntu + inputFile + " " + urlPathUbuntu + modifiedFileName + "//" + modifiedFileName + ".png");

			sr.append("\n");

			sr.append("convert \"" + urlPathUbuntu + modifiedFileName + "//" + modifiedFileName + "-0" + ".png\"" + " -thumbnail 250x90 -unsharp 0x.5 \"" + urlPathUbuntu + modifiedFileName + "//"
					+ "thumbnail" + ".png\"");
	...
	public static void addImagestoDB(String dirName, String user, String desc, String documentName) {
		...
		if (children == null) {
			...
			for (int i = 0; i < children.length; i++) {
				...
				UserImagesVO ui = new UserImagesVO(user, dirName + "/" + filename, 0);
				...
				log.debug(" length: " + children.length + "  num:  " + num);
