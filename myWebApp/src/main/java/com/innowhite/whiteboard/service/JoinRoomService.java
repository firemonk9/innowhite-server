...
public class JoinRoomService {
	...
	private static HashMap roomsMap = new HashMap();

	// @Autowired
	// static RoomNameMessageProducer roomSecurityMessageProducer;
	//
	// public static void setRoomSecurityMessageProducer(
	// RoomNameMessageProducer roomSecurityMessageProducer) {
	// JoinRoomService.roomSecurityMessageProducer =
	// roomSecurityMessageProducer;
	// }

	private static final Logger log = LoggerFactory
			.getLogger(JoinRoomService.class);
	...
	public static void main(String[] args) {
		...
		log.debug(" The .. meetingId: " + vo.getMeetingNumber() + " phone :: "
				+ vo.getPhoneNumber());

	}

	/*
	 * Returns the conference number and phone number. Unique : true if every
	 * user should get a unique number Unique : false if every user should get a
	 * same number Conference number should be 6 digit number. The first two
	 * digits uniquely represent a user.
	 */
	public static synchronized ConferenceNumbersVO setupJoinRoom(String roomId,
			String userId, boolean unique) {
	...
	private static ConferenceNumbersVO getConferenceNumberVO(String roomId,
			String userId, boolean unique) {

		log.debug("Entered getConferenceNumberVO roomId" + roomId + " userId "
				+ userId + "  unique " + unique);
		...
		// there is no body else in the conference so set up a new conference
		// number
		if (val == null) {

			ConferenceNumbersVO confVO = ConferenceMeetingDAO
					.getConfMeetingNumber();
			if (confVO != null) {

				int meetingId = Integer.parseInt("10"
						+ confVO.getMeetingNumber());
				confVO.setMeetingNumber(meetingId);
				RoomUsersDAO.save(userId, roomId, String.valueOf(meetingId));
				log.debug(" Setting the conference number for the first time meetingId: "
						+ confVO.getMeetingNumber()
						+ " phone :: "
						+ confVO.getPhoneNumber());
				return confVO;
			}
		} else {
			...
			if (conf != null) {
				...
				log.debug(" The user already exists .. meetingId: "
						+ confVO.getMeetingNumber() + " phone :: "
						+ confVO.getPhoneNumber());
				return confVO;
			} else {
				...
				if (unique == true) {
					...
					// if unique is true, the below code sends a unique number
					// starting from 101000, 111000,121000 and so on.
					...
					log.debug(" val= " + val + " fir " + firstTwoDigits
							+ "  meetingNumber " + meetingNumber);

					int meeting = Integer.parseInt(meetingNumber);
					confVO.setMeetingNumber(meeting);
					RoomUsersDAO.updateUserInRoom(userId, roomId);
					log.debug(" The user does not exists and unique is true.. meetingId: "
							+ confVO.getMeetingNumber()
							+ " phone :: "
							+ confVO.getPhoneNumber());
					...
					log.debug(" The user does not exists and unique is false.. meetingId: "
							+ confVO.getMeetingNumber()
							+ " phone :: "
							+ confVO.getPhoneNumber());

				}
				RoomUsersDAO.save(userId, roomId,
						String.valueOf(confVO.getMeetingNumber()));
				return confVO;
			}
	...
	private static RoomNameMessageProducer getMesgBean() {
		...
		RoomNameMessageProducer roomSecurityMessageProducer = (RoomNameMessageProducer) factory
				.getBean("roomSecurityMessageProducer");
		return roomSecurityMessageProducer;
	}
