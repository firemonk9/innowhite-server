	...
	public void eventReceived(final EslEvent event) {
		...
		for (final IEslEventListener listener : eventListeners) {
			...
			public void run() {
			    try {
				/**
				 * Custom extra parsing to get conference Events
				 * for BigBlueButton / FreeSwitch intergration
				 */
				...
				//    log.debug("eventFunc  :: "+eventFunc);
				    ...
				    // really.... maybe?
				    // But this way we filter whole sections of
				    // Action events
				    if (eventFunc == null) {
					// Noop...
					...
					listener.conferenceEventPlayFile(confName, confSize, event);
					return;
				    } else if (eventFunc.equals("conf_api_sub_transfer")) {
					// Member transfered to another conf...
					listener.conferenceEventTransfer(confName, confSize, event);
					return;
				    } else if (eventFunc.equals("conference_add_member")) {
					listener.conferenceEventJoin(confName, confSize, event);
					return;
				    } else if (eventFunc.equals("conference_del_member")) {
					listener.conferenceEventLeave(confName, confSize, event);
					return;
				    } else if (eventFunc.equals("conf_api_sub_mute")) {
					listener.conferenceEventMute(confName, confSize, event);
					return;
				    } else if (eventFunc.equals("conf_api_sub_unmute")) {
					listener.conferenceEventUnMute(confName, confSize, event);
					return;
				    } else if (eventFunc.equals("conference_loop_input")) {
					listener.conferenceEventAction(confName, confSize, eventHeaders.get("Action"), event);
					return;
				    } else {
					/*
					 ...
					 * sb.append("'\n"); } log.info
					 * ("Unknown Conference Event [{}] [{}]"
					 * , confName, sb.toString());
					 */
				    }
				}
