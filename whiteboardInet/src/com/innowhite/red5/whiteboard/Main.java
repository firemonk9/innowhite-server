...
import com.innowhite.red5.whiteboard.audio.FreeSwitchGateway;
import com.innowhite.red5.whiteboard.audio.FreeswitchServiceProvider;
...
public class Main extends MultiThreadedApplicationAdapter {
	...
	public boolean updateShapeEvents(com.vo.ShapeEventsVO v) {
		...
		try {
			...
			if (roomVO.getMediaDate() != null) {
				timeVal = timeVal + new Date().getTime()
						- roomVO.getMediaDate().getTime() - 2000;
			}
	...
	public boolean updateUserList(com.vo.UserListVO uservo) {
...
//						uservo.isGroupLeader()));
			...
			// if (m.get(objj) != null) {
			// // com.vo.UserListVO cannot be cast to com.vo.ShapeEventsVO
			//
	...
	public boolean updateChat(com.vo.ChatVO chatvo) {
		try {
			ISharedObject chatSO = this.getSharedObject(Red5
					.getConnectionLocal().getScope(), "ChatSO");

			int num = chatSeqMap.get(Red5.getConnectionLocal().getScope()
					.getName());
			chatvo.setSeq(num++);
			chatSeqMap.put(Red5.getConnectionLocal().getScope().getName(), num);

			chatSO.setAttribute("" + chatvo.getSeq(), chatvo);
			SavingData.saveToFile(chatvo, Red5.getConnectionLocal().getScope()
					.getName());
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public boolean updateVideo(com.vo.VideoDisplayVO videovo) {
		ISharedObject videoSO = this.getSharedObject(Red5.getConnectionLocal()
				.getScope(), "VideoDisplaySO");
		...
		log.debug(" videovo  ::" + videovo.isAudio() + "  " + videovo.isVideo()
				+ "   " + videovo.getUsername() + "  date   "
				+ roomVO.getMediaDate() + "  and seq " + videovo.getSeq()
				+ " flvpath  " + videovo.getFlvpath());

		videoSO.setAttribute("" + videovo.getSeq(), videovo);

		if (roomVO.getMediaDate() == null
				&& (videovo.isAudio() == true || videovo.isVideo() == true)) {
			roomVO.setMediaDate(new Date());
		} else if (roomVO.getMediaDate() != null
				&& (videovo.isAudio() == false && videovo.isVideo() == false)) {

			roomVO.setMediaTimeNum(roomVO.getMediaTimeNum()
					+ (new Date()).getTime()
					- (roomVO.getMediaDate().getTime()));
	...
	private void userProperCloseConnection(com.vo.VideoDisplayVO videovo,
			String roomName) {
		...
		if (videovo.getStreamType().equals("DESKTOP")) {
			if (videovo.getShareStatus().equals("STARTED")) {
				// add to the list
				if (disconnectUser.containsKey(key) == true
						&& disconnectUser.get(key) != null) {
		...
		System.err
				.println(" the HashMap of users broadcasting video/screenshare  "
						+ videovo);
	...
	private void properUserLogout(String user, String roomName) {
		...
		if (l != null && l.size() > 0) {
			for (VideoDisplayVO vidVO : l) {
				...
				ISharedObject videoSO = this.getSharedObject(Red5
						.getConnectionLocal().getScope(), "VideoDisplaySO");

				log.debug("   Why the hell is this null .... videoSO  .."
						+ videoSO);

				if (videoSO != null) {
					int num = videoSeqMap.get(Red5.getConnectionLocal()
							.getScope().getName());
					// num++;
					vidVO.setSeq(num);
					videoSO.setAttribute("" + num, vidVO);
					num++;
					videoSeqMap.put(Red5.getConnectionLocal().getScope()
							.getName(), num);

					log.debug("  putting .." + num + " as key "
							+ vidVO.getFlvpath() + "  stopped");
				} else {
			...
			try {
				...
				for (int i = 0; i < params.length; i++) {
					...
					if (params[i] != null
						...
						if (params[0] != null) {
							log.debug("  Name :  " + (String) params[0]
									+ "  client id:  " + client.getId());

							Map<String, String> map = clientNamesMap.get(conn
									.getScope().getName());
							if (map == null) {
								map = new HashMap<String, String>();
								clientNamesMap.put(conn.getScope().getName(),
										map);
							}
							map.put(client.getId(), (String) params[0]);
							UserInfo.addUser(client.getId(),
									conn.getSessionId());
							if (clientNamesMap.get(conn.getScope().getName())
									.size() > 1)
								bool = true;

							log.debug(" clientNamesMap  " + clientNamesMap
									+ " size :" + clientNamesMap.size());
				...
				// if the first user joind then open the file
				if (bool == false) {

					SavingData.openNewFile(conn.getScope().getName());
				}
	...
	public boolean roomJoin(IClient client, IScope scope) {
		...
		log.debug(" roomJoin  ###### " + client.toString() + "   "
				+ scope.getName());
	...
	public void roomLeave(IClient client, IScope room) {
		...
		try {
			...
			if (map != null) {
				...
				ISharedObject userListSO = this.getSharedObject(room,
						"UserListSO");
