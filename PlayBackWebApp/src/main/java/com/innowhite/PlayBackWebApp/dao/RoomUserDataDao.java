...
public class RoomUserDataDao {
	...
	private void updateUserRoomEndData(UserRoomData roomData) {
		...
		int val = session.createQuery(query).setTimestamp("endTime", roomData.getEndTime()).setString("roomName", roomData.getRoomName()).setString("userId", roomData.getUserId()).executeUpdate();
	...
	private void updateUserRoomStartData(UserRoomData roomData) {
		...
		int val = session.createQuery(query).setTimestamp("startTime", roomData.getStartTime()).setString("roomName", roomData.getRoomName()).setString("userId", roomData.getUserId()).executeUpdate();
	...
	private void updateRoomEndData(RoomData roomData) {
		...
		log.debug("retuned updateRoomEndData val " + val);
		// /log.debug("");
	...
	private void updateRoomStartData(RoomData roomData) {
		...
		String query = "update RoomData o set o.startTime=:startTime  and o.status=:status where o.roomName=:roomName";
		int val = session.createQuery(query).setTimestamp("startTime", roomData.getStartTime()).setString("roomName", roomData.getRoomName()).setString("status", "Not Closed Properly").executeUpdate();
