...
import com.innowhite.PlayBackWebApp.model.VideoData;
...
public class VideoDataDao {
	...
	int val = session.createQuery(query).setTimestamp("endTime", videoData.getEndTime()).
	setString("roomName", videoData.getRoomName()).executeUpdate();
	...
	try {
	    ...
	    int val = session.createQuery(query).setTimestamp("startTime", videoData.getStartTime()).
	    setString("filePath", videoData.getFilePath()).
	    setString("roomName", videoData.getRoomName()).executeUpdate();
	    // setString("videoType", "SCREEN_SHARE")
	    // setString("filePath", videoData.getFilePath())
